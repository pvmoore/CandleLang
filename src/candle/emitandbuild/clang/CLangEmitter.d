module candle.emitandbuild.clang.CLangEmitter;

import candle.all;
import candle.emitandbuild.clang.CEmitModule;

final class CLangEmitter {
public:
    this(Candle candle) {
        this.candle = candle;    
    }
    void emitAllModules() {
        ...
        foreach(m; candle.allModules()) {
            moveTopLevelTypesToModuleRoot(m);
            reorderTopLevelTypes(m);

            new CEmitModule(m).emit();
        }
    }
private:
    Candle candle;
